...
function __bobthefish_prompt_nodenv -d 'Display nodenv version'
    # Only show if nodenv is available
    type -q nodenv; or return

    # Only show in Node.js projects (check for package.json)
    test -f package.json; or return

    set -l node_version (nodenv version-name 2>/dev/null)
    test -n "$node_version"; or return
    test "$node_version" != "system"; or return

    # Use Nerd Font icon for Node.js (U+E718)
    set -l node_icon \uE718

    set_color green
    echo -n " $node_icon $node_version "
    set_color normal
end

function __bobthefish_prompt_pyenv -d 'Display pyenv version'
    # Only show if pyenv is available
    type -q pyenv; or return

    # Only show in Python projects (check for common Python files)
    test -f requirements.txt; or test -f setup.py; or test -f pyproject.toml; or test -f Pipfile; or return

    set -l python_version (pyenv version-name 2>/dev/null)
    test -n "$python_version"; or return
    test "$python_version" != "system"; or return

    # Use Nerd Font icon for Python (U+E73C)
    set -l python_icon \uE73C
...
function fish_right_prompt -d 'bobthefish is all about the right prompt'
    ...
    __bobthefish_prompt_nodenv
    __bobthefish_prompt_pyenv
